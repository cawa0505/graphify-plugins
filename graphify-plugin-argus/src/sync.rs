//! toon 封包（`.toon`）— 依 core 契約（`format_version` + `workspace_key`）。
//!
//! 比照 `graphify-plugin-review/src/sync.rs` 既有慣例：
//! - plugin 狀態放 `metadata.plugin_data["argus"]`。
...
mod tests {
    ...
    fn emit_and_parse_roundtrip() {
        let data = serde_json::json!({"argus": {"nodes": 7}});
    ...
    fn error_packet_carries_error() {
        let packet = emit_error_packet("Cannot parse .toon into GraphOutput.");
        let meta = parse_meta(&packet);
        assert_eq!(
            meta.error.as_deref(),
            Some("Cannot parse .toon into GraphOutput.")
        );
    }
