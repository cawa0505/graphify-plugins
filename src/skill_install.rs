//! Agent-skill installation (`graphify opendoc skill install`).
//!
//! Distributes the embedded [`SKILL.md`](../SKILL.md) — the dual-track opendoc
...
pub const MARKER: &str = "managed by graphify opendoc skill";
...
fn target_paths(home: &Path, cwd: &Path, agent: Agent, scope: Scope) -> Vec<PathBuf> {
    match (agent, scope) {
        ...
        (Agent::Claude, Scope::User) => vec![home.join(".claude/skills/graphify-opendoc/SKILL.md")],
        // Claude in-repo uses the repo-root canonical SKILL.md; nothing to install.
        (Agent::Claude, Scope::Project) => vec![],
        (Agent::Cursor, Scope::Project) => vec![cwd.join(".cursor/rules/graphify-opendoc.mdc")],
...
mod tests {
    ...
    fn install_writes_marked_copy() {
        ...
        assert!(content.contains("# OpenDocuments — Skill"));
        assert!(content.contains("graphify opendoc index"));
    }
    ...
    fn uninstall_skips_unmanaged_target() {
        let home = tempdir().unwrap();
        let cwd = tempdir().unwrap();
        let path = home.path().join(".claude/skills/graphify-opendoc/SKILL.md");
        fs::create_dir_all(path.parent().unwrap()).unwrap();
    ...
    fn content_marker_lands_after_frontmatter() {
        ...
        assert!(content.contains("---\n<!-- managed by graphify opendoc skill -->"));
