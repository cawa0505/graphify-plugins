...
//! - plugin 狀態放 `metadata.plugin_data["opendoc"]`（core 契約保留容器）。
...
mod tests {
    ...
    fn emit_and_parse_roundtrip() {
        let data = serde_json::json!({"opendoc": {"links": 42}});
    ...
    fn error_packet_carries_error() {
        let packet = emit_error_packet("No docs indexed. Run opendoc_index first.");
        let meta = parse_meta(&packet);
        assert_eq!(
            meta.error.as_deref(),
            Some("No docs indexed. Run opendoc_index first.")
        );
    }
